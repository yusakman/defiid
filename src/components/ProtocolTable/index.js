...
const ProtocolTable = () => {
  const protocols = useSelector((state) => state.protocols.protocols);
  ...
  const nameBodyTemplate = (rowData) => {
    return formatName(rowData.name, rowData.url);
  };

  const tvlBodyTemplate = (rowData) => {
    return formatCurrency(rowData.tvl, currency);
  };

  const mCapBodyTemplate = (rowData) => {
    return formatCurrency(rowData.mcap, currency);
  };

  const change_1dBodyTemplate = (rowData) => {
    return formatChangePrice(rowData.change_1d);
  };

  const change_7dBodyTemplate = (rowData) => {
    return formatChangePrice(rowData.change_7d);
  };
  ...
  return (
    <div className="table-wrapper" id="protokol">
      <div className="card">
        <DataTable
          dataKey="id"
          value={protocols}
          paginator
          className="p-datatable-protocols"
        ...
        >
          ...
          <Column
            field="change_1d"
            header="1D Change"
            body={change_1dBodyTemplate}
            sortable
          ></Column>
          <Column
            field="change_7d"
            header="7D Change"
            body={change_7dBodyTemplate}
            sortable
          ></Column>
